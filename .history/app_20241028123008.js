...
app.get('/admin/api/products/:id', async (req, res) => {
    ...
    const product = await Product.findOne({ ProductID: productId }); // Adjust as necessary for your database schema
    res.json(product);
});
...
app.get('/products/:id', async (req, res) => {
    try {
        const product = await Product.findById(req.params.id).populate('category'); // Assuming category is referenced
        if (!product) {
            return res.status(404).send('Product not found');
        }
        res.json(product);
