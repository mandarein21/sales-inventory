...
app.get('/admin/api/products', async (req, res) => {
    const categoryId = req.query.categoryId; // Gets the category ID from the query parameters
    console.log('Fetching products for categoryId:', categoryId); // Debugging log
    try {
        const products = await Product.find({ categoryId: categoryId }); // Fetches products that match the category ID
...
// Update product sa inventory mismo
app.put('/api/products/:id', async (req, res) => {
    const { ProductName, CategoryID, Quantity, Price } = req.body;
    const productId = req.params.id; // productId can contain letters

    try {
        const updatedProduct = await Product.findOneAndUpdate(
            { ProductID: productId }, // Find by ProductID instead of MongoDB ObjectId
            {
                ProductName,
                CategoryID: new mongoose.Types.ObjectId(CategoryID), // Convert to ObjectId here with 'new'
                Quantity,
                Price
            },
            { new: true }
...
app.delete('/sales/:saleId', async (req, res) => {
    ...
    // Debugging: log the received Sale ID
    console.log('Received Sale ID:', saleId);

    try {
        // Ensure Sale ID is an integer
        const saleIdInt = parseInt(saleId);
        console.log('Parsed Sale ID as Integer:', saleIdInt); // Debugging log
        if (isNaN(saleIdInt)) {
            return res.status(400).json({ message: 'Invalid Sale ID format' });
        }

        // Use SaleID directly to find the sale
        const result = await Sale.deleteOne({ SaleID: saleIdInt });

        // Debugging: check the result of the delete operation
