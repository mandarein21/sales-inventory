...
app.use((req, res, next) => {
    console.log(req.user); // Check what this returns
    next();
});
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        // Compare password (make sure you hash and compare correctly)
        ...
        req.user = user; // Setting req.user for middleware access
        res.redirect('/products'); // Redirect to products after login
    } catch (err) {
        console.error(err);
        res.status(500).send('Server error');
    }
