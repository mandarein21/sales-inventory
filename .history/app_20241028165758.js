...
app.get('/products', (req, res) => {
    ...
    res.render('products'); // Pass necessary data to the view
});
...
// Assuming you have some User model for authentication
app.post('/login', async (req, res) => {
    const { username, password } = req.body;

    try {
        const user = await User.findOne({ username });
        console.log('Login Attempt:', { username, user }); // Debugging user lookup
        if (!user) {
        ...
        req.user = user;
        console.log('User Logged In:', req.user); // Confirm user is set in session
        res.redirect('/products');
    } catch (err) {
        console.error(err);
        res.status(500).send('Server error');
