...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        // Set session data with Username capitalized
        req.session.admin = {
            Username: user.username, // Capitalized "Username"
            role: user.role // Add role to session data if applicable
        };

        console.log('User Logged In:', req.session.admin); // Confirm session data
...
app.get('/products', async (req, res) => {
    try {
        // Fetch products and populate the CategoryID field to get CategoryName
        ...
        // Debugging: Check req.session data
        console.log('Current Session on Products:', req.session); // Verify session info

        // Determine if the user is an admin
        const isAdmin = req.session.admin && req.session.admin.role === 'admin';

        // Set the admin object if the user is an admin
        const admin = isAdmin ? { Username: req.session.admin.username } : null;

        console.log('Admin Object:', admin); // Debugging admin object

        // Render the product view with products, categories, and admin data
...
app.post('/add-product', async (req, res) => {
    ...
    if (!productId || !productName || !CategoryID || quantity === undefined || price === undefined) {
...
//joining product and category
// Route to fetch products along with their categories
