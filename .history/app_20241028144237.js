...
app.get('/products', async (req, res) => {
    try {
        ...
        const isAdmin = req.user && req.user.role === 'admin'; // Adjust this based on your auth logic
...
app.post('/login', (req, res) => {
    ...
    // Authenticate user (adjust this logic based on your implementation)
    User.findOne({ username, password }) // Hashing passwords in practice is essential
        .then(user => {
            if (user) {
                req.session.user = user; // Store user information in session
                req.user = user; // Also set req.user for easy access
                res.redirect('/products');
            } else {
