...
router.get('/products', async (req, res) => {
    try {
        ...
        const admin = true; // or set this based on your authentication logic

        res.render('product', { products, admin }); // Render the product.ejs template with products and admin
    } catch (error) {
        console.error(error);
...
router.put('/products/update/:id', async (req, res) => {
    try {
        ...
        console.log('Updating product with ID:', productId, 'with data:', updatedProduct); // Log the update info

        const result = await Product.findOneAndUpdate({ ProductID: productId }, updatedProduct, { new: true });
