...
router.put('/:id', salesController.updateSale); // Use PUT for updates
...
router.get('/api/products/:id', async (req, res) => {
    try {
        ...
        res.json(product); // Return the entire product object
    } catch (error) {
        console.error(error);
...
router.get('/api/products', async (req, res) => {
    ...
    try {
        const objectId = mongoose.Types.ObjectId(categoryId); // Convert to ObjectId
        console.log("Converted ObjectId:", objectId);

        const products = await Product.find({ CategoryID: objectId });
        console.log("Products found:", products); // Log the products fetched
