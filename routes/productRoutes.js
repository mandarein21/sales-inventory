...
router.get('/admin/api/products', async (req, res) => {
    ...
    try {
        // Convert categoryId to an ObjectId if it is a valid MongoDB ObjectId
        const objectId = mongoose.Types.ObjectId(categoryId);
        
        // Query to find products by CategoryID
        const products = await Product.find({ "CategoryID._id": objectId });

        // Send the fetched products back as a response
        res.json(products);
    } catch (error) {
...
// Assuming you have the required modules and middleware set up
router.get('/products/:id', async (req, res) => {
    try {
        ...
        const product = await Product.findById(productId).populate('CategoryID'); // Populating if needed
